...
export async function POST(request: Request) {
  ...
  const input: AssessmentInput = {
    ...
    learningCompetency: typeof body.learningCompetency === "string" ? body.learningCompetency : "",
    itemCount: Number(body.itemCount),
    types: Array.isArray(body.types) ? body.types.filter((v): v is string => typeof v === "string").slice(0, 10) : [],
    difficulty: typeof body.difficulty === "string" ? body.difficulty : "Moderate",
  };
  ...
  try {
    const draft = await provider.generateText({
      system: "You are CoTeacher. Generate practical, age-appropriate assessment drafts. Never fabricate official curriculum requirements or learner facts. Keep the answer key consistent with the generated items.",
      user: buildAssessmentPrompt(input),
    });
    return NextResponse.json({ draft });
  } catch {
    return NextResponse.json({ error: "The AI provider could not generate the assessment." }, { status: 502 });
  }
}
