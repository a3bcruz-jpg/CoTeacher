...
type Profile = { fullName?: string; position?: string | null; gradeLevel?: string | null; department?: string | null; schoolYear?: string | null; school?: { name?: string } | null };

export default function TeacherProfileForm({ profile }: { profile: Profile | null }) {
  const [status, setStatus] = useState("");
  const [form, setForm] = useState({ fullName: profile?.fullName ?? "", position: profile?.position ?? "", school: profile?.school?.name ?? "", gradeLevel: profile?.gradeLevel ?? "", subject: "", department: profile?.department ?? "", schoolYear: profile?.schoolYear ?? "2026-2027" });
  ...
  return <form className="profile-form" onSubmit={submit}>
    ...
    <div className="form-grid"><label>Department<input value={form.department} onChange={(e) => update("department", e.target.value)} /></label><label>School year<input required value={form.schoolYear} onChange={(e) => update("schoolYear", e.target.value)} placeholder="2026-2027" /></label></div>
